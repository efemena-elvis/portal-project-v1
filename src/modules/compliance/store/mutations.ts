...
export function useComplianceMutations() {
  ...
  // COMPLIANCE FIELDS MUTATIONS
  const addComplianceField = (field: IComplianceFieldConfig) => {
    const newField = {
      ...field,
      position: complianceFields.value.length,
    };
    complianceFields.value.push(newField);
  };

  const updateComplianceField = (
    fieldId: string,
    updates: Partial<IComplianceFieldConfig>,
  ) => {
    const fieldIndex = complianceFields.value.findIndex(
      (f) => f.id === fieldId,
    );
    if (fieldIndex !== -1) {
      complianceFields.value[fieldIndex] = {
        ...complianceFields.value[fieldIndex],
        ...updates,
      };
    }
  ...
  const removeComplianceField = (fieldId: string) => {
    const fieldIndex = complianceFields.value.findIndex(
      (f) => f.id === fieldId,
    );
    if (fieldIndex !== -1) {
      complianceFields.value.splice(fieldIndex, 1);
      // Re-order positions
      complianceFields.value.forEach((field, index) => {
        field.position = index;
      });
    }
  };

  const clearComplianceFields = () => {
