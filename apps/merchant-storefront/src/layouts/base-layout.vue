...
import { BaseTopbarStore, BaseSidebar, AlertTopbar } from "@packages/uikit";
...
const getActivationStatus = () => {
  if (profileUtil.getBusinessActivatedStatus() !== "true") {
    ...
    if (profileUtil.getBusiness()?.activateMyBusiness) {
      ...
      alertTopText.value = "You are on Maintenance mode";
      alertTopActionText.value = "Complete onboarding";
      alertTopActionRoute.value = "/storefront/setup/store-details";
    }
