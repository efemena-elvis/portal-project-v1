...
const fetchRefunds = async () => {
  ...
  const refundsList = Array.isArray(data) ? data : data?.transactions || [];

  refunds.value = refundsList.map((item: Record<string, any>) => ({
...
const filteredTableBody = computed(() =>
  refunds.value.map((refund) => {
    const key = refund.status === "completed" ? "successful" : refund.status;
    const label =
      refund.status.charAt(0).toUpperCase() + refund.status.slice(1);
    return {
      date: h(TableDoubleColumn, {
        entry: {
          primaryText: getDateCreated(refund.createdAt),
          secondaryText: useDate.formatTime(refund.createdAt),
        },
      }),
      amount: getBoldTableText(refund.amount),
...
const openDetailModal = (row: RefundRow) => {
...
watch(
  ...
  () => {
    ...
    debounceTimer = setTimeout(fetchRefunds, 300);
  },
);
